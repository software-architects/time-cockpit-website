...
Get-HostedService $servicename -Certificate $cert -SubscriptionId $sub |
 ...
 Remove-Deployment -Slot 'Production' -ServiceName $servicename -SubscriptionId $sub -Certificate $cert |
    Get-OperationStatus -WaitToComplete{% endhighlight %}<p xmlns="http://www.w3.org/1999/xhtml">You'll have to plug it right after the "If Not Disable Tasks" in the build process:</p><p xmlns="http://www.w3.org/1999/xhtml">
