...
</configuration>{% endhighlight %}<h3>Create DataContext</h3><p>Now you can connect to time cockpit by creating a DataContext. Use the same username and password you used to configure time cockpit after starting it the first time.</p>{% highlight javascript %}using (var dataContext = DataContext.Create("username", "password"))
{
...
</p><p>Now it is easy to match customers in time cockpit and CRM.</p>{% highlight javascript %}// Connect to time cockpit
...
}{% endhighlight %}<p>The SetMember Method copies the value for a property from CRM into time cockpit.</p>{% highlight javascript %}static void SetMember(string crmName, string timeCockpitName, Entity crmEntity, EntityObject timeCockpitEntity)
{
