...
</p><p>After running our script we can have a look at the <em>Time Cockpit.UI.log</em> file to check if all the items have been created as expected and that the last update date is also changed accordingly.</p>{% highlight javascript %}Creating task '12' for project 'Demo Project'.
...
</p><p>The log file shows that only the modified work item has been processed by the script.</p>{% highlight javascript %}Updating title of task '12' for project 'Demo Project' from 'Simple Bug' to 'Simple Bug (added more info)'.
